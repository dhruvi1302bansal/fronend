...
import { Link } from 'react-router-dom';
import { 
  Microscope, Heart, Award, Sprout, Users, BookOpen, 
  CheckCircle, ArrowRight, HandHeart, GraduationCap, Briefcase 
} from 'lucide-react';

const AboutUs = () => {
  
  // Dummy data for the NGO/Social Tie-ups
  const partners = [
    { name: "AyurVidya Foundation", focus: "Education Funding", icon: <GraduationCap /> },
    { name: "Rural Healers Trust", focus: "Doctor Employment", icon: <Briefcase /> },
    { name: "Seva Global", focus: "Community Health", icon: <HandHeart /> },
    { name: "Veda Shiksha NGO", focus: "Scholarships", icon: <BookOpen /> },
    { name: "Roots of India", focus: "Heritage Preservation", icon: <Sprout /> },
    { name: "Jiva Karuna", focus: "Underprivileged Aid", icon: <Heart /> },
  ];
    ...
    <div className="bg-white min-h-screen font-sans">
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <div className="relative">
            <div className="absolute -top-4 -left-4 w-20 h-20 border-t-4 border-l-4 border-[#D4AF37]"></div>
            <img 
              src="https://plus.unsplash.com/premium_photo-1664475529878-7dc1ca3cdcf9?w=600&auto=format&fit=crop" 
              alt="Traditional Mortar and Pestle" 
              className="rounded-lg shadow-2xl w-full object-cover h-[500px]"
            />
            <div className="absolute -bottom-4 -right-4 w-20 h-20 border-b-4 border-r-4 border-[#D4AF37]"></div>
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12">
          ...
          <Link to="/Consulting" className="hidden md:flex items-center gap-2 text-[#D4AF37] font-bold hover:underline">
             Book a Consultation <ArrowRight size={20}/>
          </Link>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            ...
            },
            {
              ...
              spec: "Women's Health & Fertility",
              exp: "12 Years Experience",
              img: "https://images.unsplash.com/photo-1659353888906-adb3e0041693?w=600&auto=format&fit=crop"
            }
          ].map((doc, i) => (
            <div key={i} className="flex flex-col bg-white border border-stone-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow">
              <img src={doc.img} alt={doc.name} className="h-64 w-full object-cover" />
      ...
      {/* --- 5. WHY AYURVEDA WORKS --- */}
      {/* Note: Fixed text colors here slightly for better visibility on dark background elements if any, otherwise kept original structure */}
      <section className="py-20 bg-[#2F3E28] text-white">
        <div className="max-w-7xl mx-auto px-6">
           <div className="grid md:grid-cols-2 gap-12 items-center">
             <div>
               <h2 className="text-4xl font-serif text-[#D4AF37] mb-6">Why Ayurveda Works?</h2>
      ...
      {/* --- 6. IMPACT & SOCIAL TIE-UPS (NEW CAROUSEL SECTION) --- */}
      <section className="py-24 bg-white overflow-hidden relative">
        <div className="max-w-7xl mx-auto px-6 mb-16 text-center">
           <div className="flex items-center justify-center gap-2 text-[#2F3E28] font-bold tracking-widest uppercase mb-4">
              <HandHeart size={20} className="text-[#D4AF37]" /> Social Impact
           </div>
           <h2 className="text-4xl font-serif text-[#2F3E28] mb-6">
             Healing People, Empowering Communities
           </h2>
           <p className="text-gray-600 text-lg max-w-3xl mx-auto">
             We believe true wellness extends beyond the clinic. We partner with vetted NGOs 
             to fund education for underprivileged students and provide dignified employment to rural Vaidyas.
           </p>
        </div>

        {/* Infinite Scroll Carousel */}
        {/* Note: Styling for 'animate-scroll' is added via inline style tag below for simplicity */}
        <div className="relative w-full">
           {/* Gradient masks for smooth fade effect */}
           <div className="absolute left-0 top-0 z-10 h-full w-12 md:w-32 bg-gradient-to-r from-white to-transparent"></div>
           <div className="absolute right-0 top-0 z-10 h-full w-12 md:w-32 bg-gradient-to-l from-white to-transparent"></div>

           <div className="flex overflow-hidden">
             <div className="flex animate-scroll gap-8 py-4">
               {/* We duplicate the array twice to ensure seamless scrolling loop */}
               {[...partners, ...partners].map((partner, index) => (
                 <div 
                   key={index} 
                   className="flex flex-col items-center justify-center min-w-[200px] md:min-w-[250px] p-6 bg-[#F5F5DC]/50 border border-[#2F3E28]/10 rounded-xl"
                 >
                    <div className="p-4 bg-white rounded-full shadow-sm mb-4 text-[#2F3E28]">
                      {partner.icon}
                    </div>
                    <h4 className="font-serif font-bold text-[#2F3E28] text-lg">{partner.name}</h4>
                    <p className="text-sm text-[#D4AF37] font-bold uppercase tracking-wide mt-1">{partner.focus}</p>
                 </div>
               ))}
             </div>
           </div>
